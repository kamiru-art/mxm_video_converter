...
export function computePlan(): Plan {
  ...
  const timeline = positions.map((_pos, k) => ({
    pos: k + 1,
    etiqueta: rawLabels[k],
    rep: rawLabels[repOf[k]],
  }));
  const perPage = Math.max(1, s.cols * s.rows);
  const pageNumbers =
    ph1.pageNumbering === 'original'
      ? originalPageNumbers(
          printed.map((p) => p.pos),
          perPage,
          s.page_num_start,
...
async function computeDedup(statusEl: HTMLElement): Promise<void> {
  ...
  const groups = JSON.parse(
    await run('group_duplicates', { hashes, threshold: ph1.dedupThreshold }),
  ) as DedupGroups;
...
export function mountPhase1(root: HTMLElement): void {
  ...
  const dz = dropzone({
    ...
    sublabel:
      'MP4 / MOV / WebM / MKV / AVI / MPG / WMV, or PNG, JPG, TIFF, WebP (16-bit too). Nothing is uploaded.',
    ...
    onFiles: async (fileList) => {
      const videos = fileList.filter(
        (f) =>
          ...
          /\.(mp4|mov|webm|mkv|m4v|avi|mpg|mpeg|wmv|flv|3gp)$/i.test(f.name),
      );
      ...
      if (videos.length) {
        ...
        if (rejected.length) {
          toast(
            `${rejected.length} file(s) could not be read and were left out: ${rejected.slice(0, 8).join(', ')}${rejected.length > 8 ? '…' : ''}`,
            'err',
          );
        }
  ...
  includeIn.addEventListener('change', () => {
    ph1.include = includeIn.value;
    void refreshFrames();
  });
  excludeIn.addEventListener('change', () => {
    ph1.exclude = excludeIn.value;
    void refreshFrames();
  });

  const namingSel = select(
    [
      ['auto', 'Auto-increment (abc_001…)'],
      ['original', 'Original file name'],
    ],
    ph1.naming,
  );
  const numberingSel = select(
    [
      ['sequential', 'Sequential (1, 2, 3…)'],
      ['original', 'Original (position in the video)'],
    ],
    ph1.numbering,
  );
  const pageNumberingSel = select(
    [
      ['sequential', 'Sequential (1, 2, 3…)'],
      ['original', 'Original (based on the frames)'],
    ],
    ph1.pageNumbering,
  );
  ...
  namingSel.addEventListener('change', () => {
    ph1.naming = namingSel.value === 'original' ? 'original' : 'auto';
    void relabel();
  });
  numberingSel.addEventListener('change', () => {
    ph1.numbering = numberingSel.value === 'original' ? 'original' : 'sequential';
    void relabel();
  });
  pageNumberingSel.addEventListener('change', () => {
    ph1.pageNumbering = pageNumberingSel.value === 'original' ? 'original' : 'sequential';
    void refreshPreview();
  });
  ...
  function bindCheck(
    key: BooleanKeys<Settings>,
    c: { input: HTMLInputElement; label: HTMLLabelElement },
  ): HTMLLabelElement {
    c.input.checked = !!s[key];
    c.input.addEventListener('change', () => {
      s[key] = c.input.checked;
      persist();
      void refreshPreview();
    });
    binds.push(() => {
  ...
  const syncCustom = (): void => {
    customRow.style.display = paperSel.value === 'Custom' ? '' : 'none';
  };
  ...
  const modeCheck = check(
    'CYANOTYPE MODE: generate negatives for transparency film',
    isCyanotype(s),
  );
  ...
  const curveProfiles = (): SelectOption[] => [
    ['', '(no curve: linear)'],
    ...store.listProfiles('cianotipia').map((n): SelectOption => [n, n]),
  ];
  ...
  curveSel.addEventListener('change', () => {
    ...
    if (p?.ink && p.ink !== s.cyan_ink && !s.cyan_ink_stops) {
      toast(
        `Heads up: this curve was measured with ink ${p.ink} and you are now using ${s.cyan_ink}.`,
      );
    }
    persist();
    void refreshPreview();
  });
  const inkProfiles = (): SelectOption[] => [
    ['', '(plain ink)'],
    ...store.listProfiles('cianotipia_color').map((n): SelectOption => [n, n]),
  ];
  ...
  const cyanBody = el(
    ...
    field(
      'Compensation curve (cyanotype profile)',
      curveSel,
    ...
    el(
      ...
      field(
        'Curve strength (%)',
        bindNum('cyan_curve_strength', numberInput(100, { min: 0, max: 100 })),
        'How much of the calibrated curve to apply. 0 = none.',
      ),
      field(
        'Micro-contrast (%)',
        bindNum('cyan_clarity', numberInput(0, { min: 0, max: 100 })),
        'Local contrast. Brings out pencil lines.',
      ),
    ),
    field(
      'Content adaptation (%)',
      bindNum('cyan_adaptive', numberInput(0, { min: 0, max: 100 })),
    ...
    field(
      ...
      bindSel(
        ...
        select(
          [
            ['saving', 'INK-SAVING (inked halos only)'],
            ['full', 'Full (entire background inked)'],
          ],
          s.cyan_bg,
        ),
      ),
    ...
    el(
      ...
      field(
        'Blocking border (mm)',
        bindNum('cyan_frame_border_mm', numberInput(0.8, { min: 0, step: 0.1 })),
      ),
    ),
  );
  ...
  // ---------- perfil de impresora ----------
  const printerProfiles = (): SelectOption[] => [
    ['', '(no compensation)'],
    ...store.listProfiles('impresora').map((n): SelectOption => [n, n]),
  ];
  ...
  printerSel.addEventListener('change', () => {
    ...
    if (p?.marker_recomendado_mm)
      s.marker_size_mm = Math.max(s.marker_size_mm, p.marker_recomendado_mm);
    ...
    if (p)
      toast(
        `Printer profile applied (scale ${(p.scale_x * 100).toFixed(1)} % × ${(p.scale_y * 100).toFixed(1)} %).`,
        'ok',
      );
  });
  ...
  const presetOptions = (): SelectOption[] => [
    ['', '(choose a preset…)'],
    ...store.listProfiles('presets').map((n): SelectOption => [n, n]),
  ];
  const presetSel = select(presetOptions(), '');
  const presetName = el('input', { type: 'text', placeholder: 'preset name' });
  const refreshPresetList = (): void => {
    setOptions(presetSel, presetOptions());
  };
  const presetsRow = el(
    'div',
    {},
          ...
          onclick: () => {
            ...
            try {
              store.saveProfile('presets', name, {
                ...
                fase: {
                  naming: ph1.naming,
                  numbering: ph1.numbering,
                  pageNumbering: ph1.pageNumbering,
                  dedupOn: ph1.dedupOn,
                },
              });
            } catch (e) {
  ...
  genBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const settings = await resolveCyanCurve(
      ...
      const out = await generateSheets({
        settings,
        frames,
        labels: plan.printed.map((p) => p.label),
  ...
  simulateToggle.input.addEventListener('change', () => {
    ph1.previewSimulate = simulateToggle.input.checked;
    void refreshPreview();
  });
  ...
  async function refreshPreview(): Promise<void> {
    ...
    try {
      ...
      const chunk = plan.printed.slice(
        ph1.previewPage * plan.perPage,
      ...
      for (const p of chunk) {
        ...
        items.push({
          ...
          w: d.w,
          h: d.h,
          hasAlpha: project.frames[p.frameIdx].hasAlpha,
          origName: project.frames[p.frameIdx].name,
        });
      }
      const { meta, pixels } = packImageData(items);
      const first = project.frames[plan.printed[0].frameIdx];
      const pnum = plan.pageNumbers
        ? plan.pageNumbers[ph1.previewPage]
        : s.page_num_start + ph1.previewPage;
      const res = await run(
        'render_sheet',
        {
      ...
      if (info.marker_capacity && plan.numPages > info.marker_capacity) {
        avisos.push(
          `${plan.numPages} sheets exceed the ${info.marker_capacity} that marker identity can distinguish. Enable "Add a QR code per frame" in Registration: QRs identify any number of sheets.`,
        );
      }
  ...
  async function renderThumbs(): Promise<void> {
    ...
    if (ph1.dedupOn && ph1.dedupGroups) {
      ph1.dedupGroups.rep_of.forEach((r, i) => {
        if (r !== i) dupSet.add(i);
      });
    }
  ...
  function demoFrame(i: number, n: number): OffscreenCanvas {
    ...
    ctx.arc(
      58 + t * (w - 116),
      h * 0.8 - 28 - Math.abs(Math.sin(t * Math.PI * 1.5)) * h * 0.42,
      28,
      0,
      Math.PI * 2,
    );
  ...
  demoBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await afterFramesChanged();
      toast(
        `${DEMO_FRAMES} example frames ready. Press “Generate sheets”, then go to ② and simulate the scans: the whole loop without printing anything.`,
        'ok',
      );
    } catch (e) {
      console.error(e);
  ...
  const markerCountSel = select(
    [
      ['4', '4 (corners)'],
      ['8', '8 (recommended)'],
      ['12', '12 (maximum tolerance)'],
    ],
    String(s.marker_count),
  );
  ...
  binds.push(() => {
    markerCountSel.value = String(s.marker_count);
  });
  ...
  const paper = el(
    ...
    el(
      'div',
      { class: 'hint' },
      'From a video (or images) to printable sheets with registration markers.',
    ),
    dz,
    demoBtn,
    videoInfo,
    ...
    el('div', { class: 'row' }, field('Include frames', includeIn), field('Exclude', excludeIn)),
    ...
    el(
      ...
      field(
        'Digits',
        bindNum('leading_zeros', numberInput(3, { min: 1, max: 8 }), { integer: true }),
      ),
    ...
    bindCheck(
      'registration_on',
      check('ArUco markers with per-sheet identity (required for phase ②)', s.registration_on),
    ),
    el(
      'div',
      { class: 'hint' },
      'Each sheet gets its own marker IDs: no QR codes needed, more room for the drawings.',
    ),
    el(
      'div',
      { class: 'row' },
    ...
    field(
      'Project name',
      bindText('project_name', el('input', { type: 'text', placeholder: '= output name' })),
    ),
    (() => {
      const qrRow = el(
        'div',
      ...
      const sync = (): void => {
        qrRow.style.display = s.qr_on ? '' : 'none';
      };
      c.input.addEventListener('change', () => {
        s.qr_on = c.input.checked;
        sync();
        persist();
        void refreshPreview();
      });
      binds.push(() => {
        c.input.checked = !!s.qr_on;
        sync();
      });
      sync();
      return el(
        ...
        el(
          'div',
          { class: 'hint' },
          'Marker identity covers a limited number of sheets; the preview warns when a project exceeds it. QRs identify any number of sheets and keep desktop-app compatibility, at the cost of cell space.',
        ),
        qrRow,
      );
    })(),
    ...
    el('h3', {}, 'Printer'),
    field(
      'Printer profile (Calibration tab)',
      printerSel,
      'Corrects the measured scale of your printer.',
    ),
    ...
    el(
      'div',
      { class: 'row', style: 'align-items:center; margin-top:4px' },
  ...
  const bench = el(
    ...
    el(
      ...
      el('span', { class: 'preview-corner c1' }),
      el('span', { class: 'preview-corner c2' }),
      el('span', { class: 'preview-corner c3' }),
      el('span', { class: 'preview-corner c4' }),
      previewImg,
    ),
