...
//
// El ImageBitmap se transfiere al worker tal cual, sin copia ni prueba
// previa. Hubo una autocomprobación que sospechaba de Safari (la tira de
// miniaturas salía a bandas) y mandaba los píxeles crudos si fallaba. Se
// midió en Safari 26.5.2 con un MOV HEVC 4K de Lumix (241 fotogramas)
// comparando cada miniatura con una decodificación de referencia (<video> +
// canvas): el bitmap transferido llega entero y la diferencia media es la
// misma leyendo del lienzo visible o del de memoria. Las bandas eran la
// rejilla CSS de la tira, que WebKit aplastaba a filas de 6.47 px (ver
// .thumbs en style.css), no el decodificador; la prueba se quitó porque
// costaba un viaje al worker por sesión y no protegía de nada.
...
export class FrameQueue {
  ...
  async push(image: FrameSource, t: number): Promise<void> {
    const png = this.png;
    const result = run('encode_frame', { image, thumbW: THUMB_W, png }, frameTransfer(image));
