...
impl Rgb {
    ...
    pub fn fill_rect(&mut self, x1: i64, y1: i64, x2: i64, y2: i64, c: [u8; 3]) {
        ...
        let (x2, y2) = (
            (x2.max(0) as usize).min(self.w),
            (y2.max(0) as usize).min(self.h),
        );
        for y in y1..y2 {
            for x in x1..x2 {
                self.set_px(x, y, c);
...
fn build_weights(src_n: usize, dst_n: usize, f: Filter) -> Weights {
    ...
    let fscale = if scale > 1.0 || f == Filter::Area {
        scale.max(1.0)
    } else {
        1.0
    };
...
pub fn resize_rgb(src: &Rgb, dw: usize, dh: usize, f: Filter) -> Rgb {
    ...
    Rgb {
        w: dw,
        h: dh,
        data: resize_channels(&src.data, src.w, src.h, 3, dw, dh, f),
    }
}
...
pub fn resize_gray(src: &Gray, dw: usize, dh: usize, f: Filter) -> Gray {
    ...
    Gray {
        w: dw,
        h: dh,
        data: resize_channels(&src.data, src.w, src.h, 1, dw, dh, f),
    }
}

fn resize_channels_u16(
    src: &[u16],
    sw: usize,
    sh: usize,
    dw: usize,
...
mod tests {
    ...
    fn resize_rgba_flat_and_opaque() {
        let src: Vec<u8> = std::iter::repeat_n([9u8, 120, 240, 255], 40 * 30)
            .flatten()
            .collect();
